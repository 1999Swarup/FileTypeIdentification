...
model = pickle.load(open('finalftimodel.sav', 'rb'))
...
def home():
    ...
    # create a empty list, where you store the byte values
    prob_val = []
    with open("C:/Users/This PC/Others/blueOptima/all/100.kt", "rb") as f:
        text = f.read()
        c = Counter(text)
    ...
    list_of_text.append((val, freq, prob_freq, "100.kt"))
    ...
    # numerical features
    num_cols = ['OnlyFrequency', 'OnlyProbabilityDistribution']
    ...
    df = pd.get_dummies(df, columns=['OnlyValueForAbsoluteDistribution'])
